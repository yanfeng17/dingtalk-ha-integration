"""Client helper to interact with the external DingTalk gateway service."""
...
class DingTalkGatewayClient:
    ...
    async def async_send_markdown(self, target: str, title: str, content: str) -> None:
        """Send markdown message."""
        url = f"{self._base_url}/send_markdown"
        headers = self._build_headers()
        payload: Dict[str, Any] = {"target": target, "title": title, "content": content}
        try:
            async with self._session.post(url, json=payload, headers=headers, timeout=15) as resp:
                if resp.status != 200:
                    text = await resp.text()
                    raise HomeAssistantError(f"Gateway send_markdown failed: {resp.status} {text}")
        except ClientError as exc:
            raise HomeAssistantError(f"Failed to reach gateway: {exc}") from exc
    ...
    def _handle_event(self, data: Dict[str, Any]) -> None:
        ...
        try:
            ...
            _LOGGER.debug("Message event dispatched in %.2fms: %s", elapsed, data.get("content", "")[:50])
        except Exception as err:
